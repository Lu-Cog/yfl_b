							...
							<navigator v-for="goods in item.list" :key="goods.cart_id"
								...
								<view class='text'>
									...
									<view class='money'>￥{{$util.moneyAct(goods.productAttr.price)}}
										...
										<text v-else-if="goods.productAttr.activity_type==4"
											class="goods_tag">YBK</text>
										<text v-else-if="goods.productAttr.activity_type==5"
											class="goods_tag">特定价</text>
										<text v-else-if="goods.productAttr.activity_name"
											class="goods_tag">{{goods.productAttr.activity_name}}</text>
	...
	export default {
		...
		methods: {
			...
			addCart: function(goods, index) {
				...
				if (goods.price_mult == 1) {
					if (goods.cart_num < 5) {
						Number(goods.cart_num++)
						this.cartTotalCount = Number(this.cartTotalCount) + 1
					} else if (goods.cart_num < 10) {
						Number(goods.cart_num += 5)
						this.cartTotalCount = Number(this.cartTotalCount) + 5
					} else if (goods.cart_num < 50) {
						Number(goods.cart_num += 40)
						this.cartTotalCount = Number(this.cartTotalCount) + 40
					} else {
						Number(goods.cart_num += 50)
						this.cartTotalCount = Number(this.cartTotalCount) + 50
					}
				} else if (goods.price_mult == 2) {
