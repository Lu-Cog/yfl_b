<template>
	<view>
		<view class='payment-status'>
			...
			<view class='iconfont icon-iconfontguanbi bg-color icon' v-if="order_pay_info.paid == 0"></view>
			<view class='iconfont icon-duihao2 bg-color icon' v-else></view>
			<!-- <view class='iconfont icon-duihao2 bg-color icon' v-if="order_pay_info.paid !=0"></view> -->
			<!-- 失败时：订单支付失败 -->
			<view class='status' v-if="order_pay_info.paid == 0">支付失败！账户余额不足，请充值。</view>
			...
			<view class='wrapper'>
				...
				<block v-else>
					<view class='item acea-row row-between-wrapper'
						v-if="order_pay_info.paid_price > 0 && order_pay_info.total_price != order_pay_info.paid_price ">
						<view>已支付商品金额</view>
						<view class="itemCom font-color-red">￥{{$util.moneyAct(order_pay_info.paid_price )}}</view>
					</view>
					<view class='item acea-row row-between-wrapper'
						v-if="order_pay_info.total_price != order_pay_info.paid_price ">
						<view>待支付商品金额</view>
						<view class="itemCom font-color-red">
							￥{{$util.moneyAct(order_pay_info.total_price - order_pay_info.paid_price )}}</view>
					</view>
			...
			<view @tap="goto" v-if="order_pay_info.paid == 0">
				<button class='returnBnt bg-color' hover-class='none'>立即充值</button>
			</view>
			<view class="tip" v-if="order_pay_info.paid == 0">
				<view class="title">
					温馨提示:
				</view>
				<view class="content">
					您的余额不足！请及时充值，如有疑问请联系客服（新品只支持现金账户）。
				</view>
			</view>
