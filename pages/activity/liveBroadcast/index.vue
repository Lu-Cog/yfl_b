			...
			<view class='list'>
				<view v-for="(item,index) in broadcastList" :key='index'>
					<navigator hover-class="none"
						:url="((item.live_status == 103 &&  item.replay_status) || item.live_status === 101 || item.live_status === 102) && type==2 ? 'plugin-private://wx2b03c6e691cd7370/pages/live-player-plugin?room_id=' + item.room_id + '&custom_params=' + uid: ''">
						<view class='item acea-row row-between-wrapper'>
							<view class="live-image">
								<img class="image" :src="item.share_img">
								<view class="live-top" :class="item.live_status == 102 ? 'playRadius' : 'notPlayRadius'"
								...
								<view v-if="item.live_status == 101 || item.live_status == 102" class="broadcast-time">
									{{ item.show_time }}
								</view>
								<view class="broadcast_share" v-if="type==1">
									<navigator :url="'/pages/activity/liveBroadcast/liveStatistics?room_id='+item.room_id">统计</navigator>
									<navigator :url="'/pages/activity/liveShare/liveShare?room_id='+item.room_id">分享</navigator>
								</view>
							</view>
							<view class="live-wrapper">
								...
								<view class="pro-count" style="white-space: nowrap; display: flex"
									v-if="item.broadcast.length > 0">
									<navigator hover-class="none" class="item" v-for="(itm, idx) in item.broadcast"
										:key="idx">
...
<style lang="scss">
	...
	.broadcast_share navigator:first-child{
		border-right: 2rpx solid #fff;
	}
	.broadcast_share navigator{
		width: 50%;
		padding: 20rpx 0;
		text-align: center;
	}
