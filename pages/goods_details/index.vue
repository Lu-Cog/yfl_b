					...
					<view class='wrapper'>
						<view class='share acea-row row-between row-bottom'>
							<view class='money font-color'>
								￥ <text
									class='num'>{{$util.moneyAct(storeInfo.hd_price)?$util.moneyAct(storeInfo.hd_price):''}}</text>
			...
			<view class='bnt acea-row'>
				<form @submit="joinCart" report-submit='true' v-if="attr.productSelect">
					<button class='joinCart bnts' form-type="submit"
						:disabled="attr.productSelect.stock == 0 || zbSource!=''">加入购物车</button>
				</form>
	...
	export default {
		...
		onLoad(options) {
			if (options.custom_params) {
				app.globalData.spid = options.custom_params
			}
		...
		methods: {
			...
			getGoodsDetails: function() {
				...
				getProductDetail(that.id).then(res => {
					...
					if (storeInfo.activity_type == 1 || storeInfo.activity_type == 2) {
						that.activity_type_tip = storeInfo.activity_name;
					} else if (storeInfo.activity_type == 3) {
						that.activity_type_tip = "代理";
					} else if (storeInfo.activity_type == 4) {
						that.activity_type_tip = "YBK";
					} else if (storeInfo.activity_type == 5) {
						that.activity_type_tip = "特定价";
					} else if (storeInfo.activity_name) {
						that.activity_type_tip = storeInfo.activity_name;
			...
			joinCart: function(e) {
				...
				if (this.isLogin === false) {
					...
					if (this.storeInfo.now_buy == 1) {
						return uni.showToast({
							title: this.storeInfo.now_buy_msg,
							icon: 'none'
						})
					}
					this.goCat();
			...
			goCat: function(news) {
				...
				postCartAdd(q)
					.then(function(res) {
						...
						if (news) {
							uni.navigateTo({
								url: '/pages/users/order_confirm/index?cartId=' + res.data.cart_id+'&source='+that.zbSource
							});
						} else {
							that.$util.Tips({
								title: "添加购物车成功",
